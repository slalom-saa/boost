...
namespace Slalom.Boost.VisualStudio.Humanizer.Localisation.NumberToWords.Romanian
{
    ...
    class RomanianCardinalNumberConverter
    {
        ...
        public string Convert(int number, GrammaticalGender gender)
        {
            ...
            var _threeDigitParts = this.SplitEveryThreeDigits(number);
            ...
            for (var i = 0; i < _threeDigitParts.Count; i++)
            {
                ...
                var partToString = this.GetNextPartConverter(currentSet);
        ...
        private Func<int, GrammaticalGender, string> GetNextPartConverter(ThreeDigitSets currentSet)
        {
            ...
            switch (currentSet)
            {
                case ThreeDigitSets.Units:
                    converter = this.UnitsConverter;
                    break;

                case ThreeDigitSets.Thousands:
                    converter = this.ThousandsConverter;
                    break;

                case ThreeDigitSets.Millions:
                    converter = this.MillionsConverter;
                    break;

                case ThreeDigitSets.Billions:
                    converter = this.BillionsConverter;
                    break;
        ...
        private string ThreeDigitSetConverter(int number, GrammaticalGender gender, bool thisIsLastSet = false)
        {
            ...
            if (tensAndUnits <= 9)
            {
                ...
                words += " " + this.getPartByGender(_units[tensAndUnits], gender);
            }
            else if (tensAndUnits <= 19)
            {
                // special case for 'teens', from 10 to 19
                words += " " + this.getPartByGender(_teensUnder20NumberToText[tensAndUnits - 10], gender);
            }
            else
            {
                // exception for zero
                var unitsText = (units == 0 ? string.Empty : " " + (_joinGroups + " " + this.getPartByGender(_units[units], gender)));
        ...
        private string HundredsToText(int hundreds)
        {
            if (hundreds == 0)
                ...
                return this.getPartByGender(_units[hundreds], GrammaticalGender.Feminine) + " sute";
        }
        ...
        private string ThousandsConverter(int number, GrammaticalGender gender)
        {
            if (number == 0)
                ...
                return this.ThreeDigitSetConverter(number, GrammaticalGender.Feminine) + (this.IsAbove20(number) ? " " + _joinAbove20 : string.Empty) + " mii";
        }
        ...
        */
        ...
        private string MillionsConverter(int number, GrammaticalGender gender)
        {
            if (number == 0)
                ...
                return this.ThreeDigitSetConverter(number, GrammaticalGender.Feminine, true) + (this.IsAbove20(number) ? " " + _joinAbove20 : string.Empty) + " milioane";
        }
        ...
        private string BillionsConverter(int number, GrammaticalGender gender)
        {
            if (number == 1)
                ...
                return this.ThreeDigitSetConverter(number, GrammaticalGender.Feminine) + (this.IsAbove20(number) ? " " + _joinAbove20 : string.Empty) + " miliarde";
        }
    }
}