...
namespace Slalom.Boost.VisualStudio.Humanizer.Localisation.NumberToWords
{
    internal class HebrewNumberToWordsConverter : GenderedNumberToWordsConverter
    {
        ...
        {
            if (number < 0)
                return string.Format("מינוס {0}", this.Convert(-number, gender));
            ...
            {
                this.ToBigNumber(number, Group.Billions, parts);
                number %= (int)Group.Billions;
            }

            if (number >= (int)Group.Millions)
            {
                this.ToBigNumber(number, Group.Millions, parts);
            ...
            if (number > 0)
            {
                ...
                {
                    var unit = this.Convert(number % 10, gender);
        ...
        private void ToBigNumber(int number, Group group, List<string> parts)
        {
            ...
            if (digits == 2)
                ...
                parts.Add(this.Convert(digits, GrammaticalGender.Masculine));
        ...
        private void ToThousands(int number, List<string> parts)
        {
            ...
            if (thousands == 1)
                ...
                parts.Add(this.Convert(thousands) + " אלף");
        }
