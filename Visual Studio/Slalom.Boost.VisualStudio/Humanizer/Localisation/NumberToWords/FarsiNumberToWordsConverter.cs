...
namespace Slalom.Boost.VisualStudio.Humanizer.Localisation.NumberToWords
{
    internal class FarsiNumberToWordsConverter : GenderlessNumberToWordsConverter
    {
        ...
        {
            if (number < 0)
                return string.Format("منفی {0}", this.Convert(-number));
            ...
            { 
                {(int)Math.Pow(10, 9), n => string.Format("{0} میلیارد", this.Convert(n)) },
                {(int)Math.Pow(10, 6), n => string.Format("{0} میلیون", this.Convert(n)) },
                {(int)Math.Pow(10, 3), n => string.Format("{0} هزار", this.Convert(n)) },
        ...
        public override string ConvertToOrdinal(int number)
        {
            ...
            if (number % 10 == 3 && number != 13)
                return this.Convert((number / 10) * 10) + " و سوم";
