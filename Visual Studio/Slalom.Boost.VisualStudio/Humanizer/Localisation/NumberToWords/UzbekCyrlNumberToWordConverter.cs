...
namespace Slalom.Boost.VisualStudio.Humanizer.Localisation.NumberToWords
{
    internal class UzbekCyrlNumberToWordConverter : GenderlessNumberToWordsConverter
    {
        ...
        {
            if (number < 0)
                return string.Format("минус {0}", this.Convert(-number, true));
        ...
        private string Convert(int number, bool checkForHoundredRule)
        {
            ...
            {
                sb.AppendFormat("{0} миллиард ", this.Convert(number / 1000000000, false));
                number %= 1000000000;
            }

            if ((number / 1000000) > 0)
            {
                sb.AppendFormat("{0} миллион ", this.Convert(number / 1000000, true));
                number %= 1000000;
            }
            ...
            if (thousand > 0)
            {
                sb.AppendFormat("{0} минг ", this.Convert(thousand, true));
                number %= 1000;
            }
            ...
            if (hundred > 0)
            {
                sb.AppendFormat("{0} юз ", this.Convert(hundred, false));
                number %= 100;
            }
