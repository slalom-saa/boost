...
namespace Slalom.Boost.VisualStudio.Humanizer.Inflections
{
    /// <summary>
    ...
    public class Vocabulary
    {
        ...
        {
            var result = this.ApplyRules(_plurals, word);

            if (inputIsKnownToBeSingular)
                return result;

            var asSingular = this.ApplyRules(_singulars, word);
            var asSingularAsPlural = this.ApplyRules(_plurals, asSingular);
        ...
        {
            var result = this.ApplyRules(_singulars, word);

            if (inputIsKnownToBePlural)
                return result;

            // the Plurality is unknown so we should check all possibilities
            var asPlural = this.ApplyRules(_plurals, word);
            var asPluralAsSingular = this.ApplyRules(_singulars, asPlural);
