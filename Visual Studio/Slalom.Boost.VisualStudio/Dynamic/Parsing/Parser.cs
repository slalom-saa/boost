...
using System.Text;
using Slalom.Boost.VisualStudio.Dynamic.Exceptions;
...
namespace Slalom.Boost.VisualStudio.Dynamic.Parsing
{
	internal class Parser
	{
		...
		Expression Parse()
		{
			Expression expr = this.ParseExpressionSegment(_arguments.ExpressionReturnType);

			this.ValidateToken(TokenId.End, ErrorMessages.SyntaxError);
			return expr;
		}
		...
		Expression ParseExpressionSegment(Type returnType)
		{
			...
			var expression = this.ParseExpressionSegment();

			if (returnType != typeof(void))
			{
				return this.GenerateConversion(expression, returnType, errorPos);
			}
		...
		Expression ParseAssignement()
		{
			...
			if (_token.id == TokenId.Equal)
			{
				...
				Token op = _token;
				this.NextToken();
				Expression right = this.ParseAssignement();
				this.CheckAndPromoteOperands(typeof(ParseSignatures.IEqualitySignatures), op.text, ref left, ref right, op.pos);
		...
		Expression ParseConditional()
		{
			...
			{
				this.NextToken();
				Expression expr1 = this.ParseExpressionSegment();
				this.ValidateToken(TokenId.Colon, ErrorMessages.ColonExpected);
				this.NextToken();
				Expression expr2 = this.ParseExpressionSegment();
		...
		Expression ParseLogicalOr()
		{
			...
			while (_token.id == TokenId.DoubleBar)
			{
				...
				this.CheckAndPromoteOperands(typeof(ParseSignatures.ILogicalSignatures), op.text, ref left, ref right, op.pos);
		...
		{
			Expression left = this.ParseTypeTesting();
					...
					_token.id == TokenId.LessThan || _token.id == TokenId.LessThanEqual)
			{
				...
				this.CheckAndPromoteOperands(isEquality ? typeof(ParseSignatures.IEqualitySignatures) : typeof(ParseSignatures.IRelationalSignatures),
					op.text, ref left, ref right, op.pos);
		...
		Expression ParseTypeTesting()
		{
					...
					|| _token.text == ParserConstants.KEYWORD_AS)
			{
				...
				if (!_arguments.TryGetKnownType(_token.text, out knownType))
					throw this.CreateParseException(op.pos, ErrorMessages.TypeIdentifierExpected);
				...
				if (typeOperator == ParserConstants.KEYWORD_IS)
					...
					throw this.CreateParseException(_token.pos, ErrorMessages.SyntaxError);
			...
			while (_token.id == TokenId.Plus || _token.id == TokenId.Minus)
			{
				...
				switch (op.id)
				{
					case TokenId.Plus:
						...
						{
							this.CheckAndPromoteOperands(typeof(ParseSignatures.IAddSignatures), op.text, ref left, ref right, op.pos);
							left = this.GenerateAdd(left, right);
						}
						break;
					case TokenId.Minus:
						this.CheckAndPromoteOperands(typeof(ParseSignatures.ISubtractSignatures), op.text, ref left, ref right, op.pos);
		...
		Expression ParseMultiplicative()
		{
					...
					_token.id == TokenId.Percent)
			{
				...
				this.CheckAndPromoteOperands(typeof(ParseSignatures.IArithmeticSignatures), op.text, ref left, ref right, op.pos);
		...
		Expression ParseUnary()
		{
			if (_token.id == TokenId.Minus || _token.id == TokenId.Exclamation || _token.id == TokenId.Plus)
			{
				...
				if (op.id == TokenId.Minus)
				{
					this.CheckAndPromoteOperand(typeof(ParseSignatures.INegationSignatures), op.text, ref expr, op.pos);
				...
				{
					this.CheckAndPromoteOperand(typeof(ParseSignatures.INotSignatures), op.text, ref expr, op.pos);
		...
		Expression ParsePrimary()
		{
			...
			while (true)
			{
				...
				else if (_token.id == TokenId.OpenParen)
				{
					...
					if (lambda != null)
						return this.ParseLambdaInvocation(lambda, tokenPos);
					else if (typeof(Delegate).IsAssignableFrom(expr.Type))
						expr = this.ParseDelegateInvocation(expr, tokenPos);
					else
						throw this.CreateParseException(tokenPos, ErrorMessages.InvalidMethodCall, GetTypeName(expr.Type));
				}
				else
				{
					break;
		...
		Expression ParsePrimaryStart()
		{
			switch (_token.id)
			{
				...
				default:
					throw this.CreateParseException(_token.pos, ErrorMessages.ExpressionExpected);
			}
		}

		Expression ParseCharLiteral()
		{
			this.ValidateToken(TokenId.CharLiteral);
			string s = _token.text.Substring(1, _token.text.Length - 2);

			s = this.EvalEscapeStringLiteral(s);

			if (s.Length != 1)
				throw this.CreateParseException(_token.pos, ErrorMessages.InvalidCharacterLiteral);
			...
			for (int i = 0; i < source.Length; i++)
			{
				...
				if (c == '\\')
				{
					if ((i + 1) == source.Length)
						throw this.CreateParseException(_token.pos, ErrorMessages.InvalidEscapeSequence);

					builder.Append(this.EvalEscapeChar(source[++i]));
				}
				else
					builder.Append(c);
			}
		...
		Expression ParseIntegerLiteral()
		{
			...
			if (text[0] != '-')
			{
				...
				if (!UInt64.TryParse(text, ParseLiteralUnsignedNumberStyle, ParseCulture, out value))
					throw this.CreateParseException(_token.pos, ErrorMessages.InvalidIntegerLiteral, text);
		...
		Expression ParseRealLiteral()
		{
			...
			if (value == null)
				throw this.CreateParseException(_token.pos, ErrorMessages.InvalidRealLiteral, text);
		...
		{
			this.ValidateToken(TokenId.OpenParen, ErrorMessages.OpenParenExpected);
			this.NextToken();
			Expression innerParenthesesExpression = this.ParseExpressionSegment();
			this.ValidateToken(TokenId.CloseParen, ErrorMessages.CloseParenOrOperatorExpected);

			var constExp = innerParenthesesExpression as ConstantExpression;
			if (constExp != null && constExp.Value is Type)
			{
				this.NextToken();
				var nextExpression = this.ParseExpressionSegment();
		...
		Expression ParseIdentifier()
		{
			...
			if (_arguments.TryGetKnownType(_token.text, out knownType))
			{
				return this.ParseTypeKeyword(knownType);
			}
		...
		Expression ParseTypeof()
		{
			...
			if (args.Length != 1)
				throw this.CreateParseException(errorPos, ErrorMessages.TypeofRequiresOneArg);
			var constExp = args[0] as ConstantExpression;
			if (!(constExp.Type is Type))
				throw this.CreateParseException(errorPos, ErrorMessages.TypeofRequiresAType);
		...
		Expression GenerateConditional(Expression test, Expression expr1, Expression expr2, int errorPos)
		{
			if (test.Type != typeof(bool))
				throw this.CreateParseException(errorPos, ErrorMessages.FirstExprMustBeBool);
			if (expr1.Type != expr2.Type)
			{
				Expression expr1as2 = expr2 != ParserConstants.NULL_LITERAL_EXPRESSION ? this.PromoteExpression(expr1, expr2.Type, true) : null;
				Expression expr2as1 = expr1 != ParserConstants.NULL_LITERAL_EXPRESSION ? this.PromoteExpression(expr2, expr1.Type, true) : null;
				...
				else
				{
					...
					if (expr1as2 != null && expr2as1 != null)
						throw this.CreateParseException(errorPos, ErrorMessages.BothTypesConvertToOther, type1, type2);
					throw this.CreateParseException(errorPos, ErrorMessages.NeitherTypeConvertsToOther, type1, type2);
				}
			}
		...
		{
			this.NextToken();
			this.ValidateToken(TokenId.Identifier, ErrorMessages.IdentifierExpected);
			...
			if (constructor == null)
				throw this.CreateParseException(_token.pos, ErrorMessages.NoApplicableConstructor, newType);
		...
		Expression ParseLambdaInvocation(LambdaExpression lambda, int errorPos)
		{
			...
			if (!this.PrepareDelegateInvoke(lambda.Type, ref args))
				throw this.CreateParseException(errorPos, ErrorMessages.ArgsIncompatibleWithLambda);
		...
		Expression ParseDelegateInvocation(Expression delegateExp, int errorPos)
		{
			...
			if (!this.PrepareDelegateInvoke(delegateExp.Type, ref args))
				throw this.CreateParseException(errorPos, ErrorMessages.ArgsIncompatibleWithDelegate);
		...
		{
			var applicableMethods = this.FindMethods(type, "Invoke", false, args);
		...
		Expression ParseTypeKeyword(Type type)
		{
			...
			if (_token.id == TokenId.Question)
			{
				if (!type.IsValueType || IsNullableType(type))
					throw this.CreateParseException(errorPos, ErrorMessages.TypeHasNoNullableForm, GetTypeName(type));
			...
			this.ValidateToken(TokenId.Dot, ErrorMessages.DotOrOpenParenExpected);
			this.NextToken();
		...
		Expression GenerateConversion(Expression expr, Type type, int errorPos)
		{
			...
			{
				throw this.CreateParseException(errorPos, ErrorMessages.CannotConvertValue,
		...
		Expression ParseMemberAccess(Type type, Expression instance)
		{
			...
			{
				return this.ParseMethodInvocation(type, instance, errorPos, id);
			}

			return this.GeneratePropertyOrFieldExpression(type, instance, errorPos, id);
		}

		Expression GeneratePropertyOrFieldExpression(Type type, Expression instance, int errorPos, string propertyOrFieldName)
		{
			MemberInfo member = this.FindPropertyOrField(type, propertyOrFieldName, instance == null);
			if (member != null)
			{
			...
			throw this.CreateParseException(errorPos, ErrorMessages.UnknownPropertyOrField, propertyOrFieldName, GetTypeName(type));
		}
		...
		Expression ParseMethodInvocation(Type type, Expression instance, int errorPos, string methodName)
		{
			...
			var methodInvocationExpression = this.ParseNormalMethodInvocation(type, instance, errorPos, methodName, args);

			if (methodInvocationExpression == null && instance != null)
			{
				methodInvocationExpression = this.ParseExtensionMethodInvocation(type, instance, errorPos, methodName, args);
			}
		...
		Expression ParseExtensionMethodInvocation(Type type, Expression instance, int errorPos, string id, Expression[] args)
		{
			...
			var extensionMethods = this.FindExtensionMethods(type, id, extensionMethodsArguments);
			if (extensionMethods.Length > 1)
				throw this.CreateParseException(errorPos, ErrorMessages.AmbiguousMethodInvocation, id, GetTypeName(type));
		...
		{
			var applicableMethods = this.FindMethods(type, id, instance == null, args);
			if (applicableMethods.Length > 1)
				throw this.CreateParseException(errorPos, ErrorMessages.AmbiguousMethodInvocation, id, GetTypeName(type));
		...
		Expression[] ParseArgumentList()
		{
			this.ValidateToken(TokenId.OpenParen, ErrorMessages.OpenParenExpected);
			this.NextToken();
			Expression[] args = _token.id != TokenId.CloseParen ? this.ParseArguments() : new Expression[0];
			this.ValidateToken(TokenId.CloseParen, ErrorMessages.CloseParenOrCommaExpected);
			this.NextToken();
			return args;
		}
		...
		Expression[] ParseArguments()
		{
			...
			while (true)
			{
				argList.Add(this.ParseExpressionSegment());
		...
		{
			int errorPos = _token.pos;
			this.ValidateToken(TokenId.OpenBracket, ErrorMessages.OpenParenExpected);
			this.NextToken();
			Expression[] args = this.ParseArguments();
			this.ValidateToken(TokenId.CloseBracket, ErrorMessages.CloseBracketOrCommaExpected);
			this.NextToken();
			if (expr.Type.IsArray)
			{
				if (expr.Type.GetArrayRank() != 1 || args.Length != 1)
					throw this.CreateParseException(errorPos, ErrorMessages.CannotIndexMultiDimArray);
				Expression index = this.PromoteExpression(args[0], typeof(int), true);
				if (index == null)
					throw this.CreateParseException(errorPos, ErrorMessages.InvalidIndex);
				return Expression.ArrayIndex(expr, index);
			}
			else
			{
				var applicableMethods = this.FindIndexer(expr.Type, args);
				if (applicableMethods.Length == 0)
				{
					throw this.CreateParseException(errorPos, ErrorMessages.NoApplicableIndexer,
							GetTypeName(expr.Type));
				}

				if (applicableMethods.Length > 1)
				{
					throw this.CreateParseException(errorPos, ErrorMessages.AmbiguousIndexerInvocation,
							GetTypeName(expr.Type));
				}
		...
		void CheckAndPromoteOperand(Type signatures, string opName, ref Expression expr, int errorPos)
		{
			...
			args = this.PrepareOperandArguments(signatures, args);
		...
		{
			var applicableMethods = this.FindMethods(signatures, "F", false, args);
			if (applicableMethods.Length == 1)
		...
		MethodData[] FindMethods(Type type, string methodName, bool staticAccess, Expression[] args)
		{
			...
			foreach (Type t in SelfAndBaseTypes(type))
			{
				...
				var applicableMethods = this.FindBestMethod(members.Cast<MethodBase>(), args);
		...
		MethodData[] FindExtensionMethods(Type type, string methodName, Expression[] args)
		{
			...
			return this.FindBestMethod(matchMethods.Cast<MethodBase>(), args);
		}
		...
		MethodData[] FindIndexer(Type type, Expression[] args)
		{
			foreach (Type t in SelfAndBaseTypes(type))
			{
				...
				if (members.Length != 0)
				{
					...
					var applicableMethods = this.FindBestMethod(methods, args);
					if (applicableMethods.Length > 0)
						return applicableMethods;
				}
			}
		...
		MethodData[] FindBestMethod(IEnumerable<MethodBase> methods, Expression[] args)
		{
			MethodData[] applicable = methods.
					...
					Where(m => this.CheckIfMethodIsApplicableAndPrepareIt(m, args)).
					ToArray();
		...
		bool CheckIfMethodIsApplicableAndPrepareIt(MethodData method, Expression[] args)
		{
			...
			foreach (var currentArgument in args)
			{
				...
				if (paramsArrayPromotedArgument == null)
				{
					...
					{
						var promoted = this.PromoteExpression(currentArgument, parameterType, true);
						if (promoted != null)
						{
				...
				if (paramsArrayTypeFound != null)
				{
					var promoted = this.PromoteExpression(currentArgument, paramsArrayTypeFound.GetElementType(), true);
					if (promoted != null)
					{
					...
					method.MethodBase is MethodInfo)
			{
				...
				var genericArgsType = this.ExtractActualGenericArguments(
		...
		List<Type> ExtractActualGenericArguments(Type[] requestedParameters, Type[] actualParameters)
		{
			...
			for (int i = 0; i < requestedParameters.Length; i++)
			{
				...
				{
					var innerGenericTypes = this.ExtractActualGenericArguments(requestedType.GetGenericArguments(), actualType.GetGenericArguments());
		...
		void PreviousChar()
		{
			this.SetTextPos(_parsePosition - 1);
		}
		...
		void NextToken()
		{
			...
			switch (_parseChar)
			{
				...
				case '&':
					...
					{
						throw this.CreateParseException(_parsePosition, ErrorMessages.InvalidCharacter, _parseChar);
					}
					break;
				case '(':
					this.NextChar();
				...
				case '"':
					...
					if (_parsePosition == _expressionTextLength)
						throw this.CreateParseException(_parsePosition, ErrorMessages.UnterminatedStringLiteral);
		...
		{
			if (!char.IsDigit(_parseChar))
				throw this.CreateParseException(_parsePosition, ErrorMessages.DigitExpected);
		}

		void ValidateToken(TokenId t, string errorMessage)
		{
			if (_token.id != t)
				throw this.CreateParseException(_token.pos, errorMessage);
		}
