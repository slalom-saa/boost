...
namespace Slalom.Boost.Humanizer.Localisation.NumberToWords
{
    internal class TurkishNumberToWordConverter : GenderlessNumberToWordsConverter
    {
        ...
        public override string Convert(int number)
        {
            ...
            if (number < 0)
                return string.Format("eksi {0}", this.Convert(-number));

            var parts = new List<string>();

            if ((number / 1000000000) > 0)
            {
                parts.Add(string.Format("{0} milyar", this.Convert(number / 1000000000)));
                number %= 1000000000;
            }

            if ((number / 1000000) > 0)
            {
                parts.Add(string.Format("{0} milyon", this.Convert(number / 1000000)));
                number %= 1000000;
            }
            ...
            if (thousand > 0)
            {
                parts.Add(string.Format("{0} bin", thousand > 1 ? this.Convert(thousand) : "").Trim());
                number %= 1000;
            }
            ...
            if (hundred > 0)
            {
                parts.Add(string.Format("{0} yüz", hundred > 1 ? this.Convert(hundred) : "").Trim());
                number %= 100;
            }
