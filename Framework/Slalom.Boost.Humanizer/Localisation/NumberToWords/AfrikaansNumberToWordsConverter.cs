...
namespace Slalom.Boost.Humanizer.Localisation.NumberToWords
{
    internal class AfrikaansNumberToWordsConverter : GenderlessNumberToWordsConverter
    {
        ...
        private string Convert(int number, bool isOrdinal)
        {
            ...
            {
                parts.Add(string.Format("{0} miljard", this.Convert(number / 1000000000)));
                number %= 1000000000;
            }

            if ((number / 1000000) > 0)
            {
                parts.Add(string.Format("{0} miljoen", this.Convert(number / 1000000)));
                number %= 1000000;
            }

            if ((number / 1000) > 0)
            {
                parts.Add(string.Format("{0} duisend", this.Convert(number / 1000)));
                number %= 1000;
            }

            if ((number / 100) > 0)
            {
                parts.Add(string.Format("{0} honderd", this.Convert(number / 100)));
                number %= 100;
            }
