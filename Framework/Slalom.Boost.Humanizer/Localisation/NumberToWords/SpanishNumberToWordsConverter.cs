...
namespace Slalom.Boost.Humanizer.Localisation.NumberToWords
{
    internal class SpanishNumberToWordsConverter : GenderedNumberToWordsConverter
    {
        ...
        public override string Convert(int number, GrammaticalGender gender)
        {
            ...
            if (number < 0)
                return string.Format("menos {0}", this.Convert(Math.Abs(number)));
            ...
            if ((number / 1000000000) > 0)
            {
                parts.Add(number / 1000000000 == 1
                    ? "mil millones"
                    : string.Format("{0} mil millones", this.Convert(number / 1000000000)));
            ...
            if ((number / 1000000) > 0)
            {
                parts.Add(number / 1000000 == 1
                    ? "un millón"
                    : string.Format("{0} millones", this.Convert(number / 1000000)));
            ...
            if ((number / 1000) > 0)
            {
                parts.Add(number / 1000 == 1
                    ? "mil"
                    : string.Format("{0} mil", this.Convert(number / 1000, gender)));
