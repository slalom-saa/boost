...
namespace Slalom.Boost.Humanizer.Localisation.NumberToWords
{
    internal class BanglaNumberToWordsConverter : GenderlessNumberToWordsConverter
    {
        ...
        public override string Convert(int number)
        {
            ...
            if (number < 0)
                return string.Format("ঋণাত্মক {0}", this.Convert(-number));

            var parts = new List<string>();

            if ((number/10000000) > 0)
            {
                parts.Add(string.Format("{0} কোটি", this.Convert(number/10000000)));
                number %= 10000000;
            }

            if ((number/100000) > 0)
            {
                parts.Add(string.Format("{0} লক্ষ", this.Convert(number/100000)));
                number %= 100000;
            }

            if ((number/1000) > 0)
            {
                parts.Add(string.Format("{0} হাজার", this.Convert(number/1000)));
                number %= 1000;
            }
