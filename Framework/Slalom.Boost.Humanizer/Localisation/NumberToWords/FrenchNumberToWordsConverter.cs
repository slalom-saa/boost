...
namespace Slalom.Boost.Humanizer.Localisation.NumberToWords
{
    internal class FrenchNumberToWordsConverter : GenderedNumberToWordsConverter
    {
        ...
        public override string Convert(int number, GrammaticalGender gender)
        {
            ...
            if (number < 0)
                return string.Format("moins {0}", this.Convert(Math.Abs(number)));
            ...
            if ((number / 1000000000) > 0)
            {
                parts.Add(string.Format("{0} milliard{1}",
                    this.Convert(number/1000000000),
            ...
            if ((number / 1000) > 0)
            {
                parts.Add(number/1000 == 1
                    ? string.Format("mille")
                    : string.Format("{0} mille", this.Convert(number/1000)));
            ...
            if ((number / 100) > 0)
            {
                parts.Add(number < 200 ? "cent" : string.Format("{0} cent", this.Convert(number/100)));
                number %= 100;
            }
            ...
            if (number > 0)
            {
                ...
                else
                {
                    ...
                    if (number >= 70 && (number < 80 || number >= 90))
                    {
                        ...
                        lastPart = string.Format("{0}-{1}", TensMap[baseNumber/10], this.Convert(number - baseNumber));
                    }
                    else
                    {
        ...
        public override string ConvertToOrdinal(int number, GrammaticalGender gender)
        {
            ...
            var convertedNumber = this.Convert(number);
