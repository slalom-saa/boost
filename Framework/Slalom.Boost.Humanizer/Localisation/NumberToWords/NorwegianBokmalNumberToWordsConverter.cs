...
namespace Slalom.Boost.Humanizer.Localisation.NumberToWords
{
    internal class NorwegianBokmalNumberToWordsConverter : GenderedNumberToWordsConverter
    {
        ...
        {
            return this.Convert(number, false, gender);
        }
        ...
        private string Convert(int number, bool isOrdinal, GrammaticalGender gender)
        {
            ...
            if (number < 0)
                return string.Format("minus {0}", this.Convert(-number, isOrdinal, gender));
            ...
            if ((number / billion) > 0)
            {
                ...
                parts.Add(this.Part("{0} milliard" + (isExactOrdinal ? "" : "er"), (isExactOrdinal ? "" : "en ") + "milliard", number / billion, !isExactOrdinal));
                number %= billion;
            }
            ...
            if ((number / million) > 0)
            {
                ...
                parts.Add(this.Part("{0} million" + (isExactOrdinal ? "" : "er"), (isExactOrdinal ? "" : "en ") + "million", number / million, !isExactOrdinal));
                number %= million;
            }
            ...
            {
                thousand = true;
                parts.Add(this.Part("{0}tusen", number % 1000 < 100 ? "tusen" : "ettusen", number / 1000));
                number %= 1000;
            }
            ...
            {
                hundred = true;
                parts.Add(this.Part("{0}hundre", thousand || millionOrMore ? "ethundre" : "hundre", number / 100));
                number %= 100;
            }
        ...
        private string Part(string pluralFormat, string singular, int number, bool postfixSpace = false)
        {
            ...
            return string.Format(pluralFormat, this.Convert(number)) + postfix;
        }
    }
}