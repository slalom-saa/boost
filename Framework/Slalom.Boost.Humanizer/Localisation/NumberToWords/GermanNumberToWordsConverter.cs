...
namespace Slalom.Boost.Humanizer.Localisation.NumberToWords
{
    internal class GermanNumberToWordsConverter : GenderedNumberToWordsConverter
    {
        ...
        public override string Convert(int number, GrammaticalGender gender)
        {
            ...
            if (number < 0)
                return string.Format("minus {0}", this.Convert(-number));
        ...
        public override string ConvertToOrdinal(int number, GrammaticalGender gender)
        {
            ...
            if (billions > 0)
            {
                ...
                parts.Add(this.Part(BillionOrdinalPlural[noRest], BillionOrdinalSingular[noRest], billions));
            }
            ...
            if (millions > 0)
            {
                number %= 1000000;
                var noRest = NoRestIndex(number);
                parts.Add(this.Part(MillionOrdinalPlural[noRest], MillionOrdinalSingular[noRest], millions));
            }
            ...
            if (number > 0)
                parts.Add(number < 20 ? UnitsOrdinal[number] : this.Convert(number));
        ...
        private string Part(string pluralFormat, string singular, int number)
        {
            if (number == 1)
                return singular;
            return string.Format(pluralFormat, this.Convert(number));
        }
