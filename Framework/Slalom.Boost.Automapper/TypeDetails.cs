...
namespace Slalom.Boost.AutoMapper
{
    ...
    public class TypeDetails
    {
        ...
        {
            this.Type = type;
            var membersToMap = this.MembersToMap(shouldMapProperty, shouldMapField);
            var publicReadableMembers = this.GetAllPublicReadableMembers(membersToMap);
            var publicWritableMembers = this.GetAllPublicWritableMembers(membersToMap);
            this.PublicReadAccessors = BuildPublicReadAccessors(publicReadableMembers);
            this.PublicWriteAccessors = BuildPublicAccessors(publicWritableMembers);
            this.PublicNoArgMethods = this.BuildPublicNoArgMethods();
            this.Constructors = type.GetDeclaredConstructors().Where(ci => !ci.IsStatic).ToArray();
        ...
        private IList<MethodInfo> BuildPublicNoArgExtensionMethods(IEnumerable<MethodInfo> sourceExtensionMethodSearch)
        {
            ...
            var genericInterfaces = this.Type.GetTypeInfo().ImplementedInterfaces.Where(t => t.IsGenericType()).ToList();
        ...
        {
            return this.GetAllPublicMembers(PropertyReadable, this.FieldReadable, membersToMap);
        }

        private IEnumerable<MemberInfo> GetAllPublicWritableMembers(Func<MemberInfo, bool> membersToMap)
        {
            return this.GetAllPublicMembers(PropertyWritable, this.FieldWritable, membersToMap);
        }
            ...
            Func<MemberInfo, bool> memberAvailableFor)
        {
            ...
            for (var t = this.Type; t != null; t = t.BaseType())
                typesToScan.Add(t);

            if (this.Type.IsInterface())
                typesToScan.AddRange(this.Type.GetTypeInfo().ImplementedInterfaces);
