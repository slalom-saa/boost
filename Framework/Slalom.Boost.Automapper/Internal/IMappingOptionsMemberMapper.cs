...
namespace Slalom.Boost.AutoMapper.Internal
{
    ...
    public class ParentSourceToDestinationNameMapper : IParentSourceToDestinationNameMapper
    {
        ...
        public MemberInfo GetMatchingMemberInfo(TypeDetails typeInfo, Type destType, string nameToSearch)
        {
            ...
            foreach (var namedMapper in this.NamedMappers)
            {
                memberInfo = namedMapper.GetMatchingMemberInfo(this.GetMembers, typeInfo, destType, nameToSearch);
                if (memberInfo != null)
                    break;
            }
            return memberInfo;
        ...
        public MemberInfo GetMatchingMemberInfo(IGetTypeInfoMembers getTypeInfoMembers, TypeDetails typeInfo, Type destType, string nameToSearch)
        {
            return
                getTypeInfoMembers.GetMemberInfos(typeInfo)
                    .FirstOrDefault(
                        mi =>
                            typeof (ParameterInfo).IsAssignableFrom(destType) || !this.MethodCaseSensitive
    ...
    public class PrePostfixName : ISourceToDestinationNameMapper
    {
        ...
        public MemberInfo GetMatchingMemberInfo(IGetTypeInfoMembers getTypeInfoMembers, TypeDetails typeInfo, Type destType, string nameToSearch)
        {
            var possibleSourceNames = this.PossibleNames(nameToSearch, this.DestinationPrefixes, this.DestinationPostfixes);
            var possibleDestNames = getTypeInfoMembers.GetMemberInfos(typeInfo).Select(mi => new { mi, possibles = this.PossibleNames(mi.Name, this.Prefixes, this.Postfixes) });
        ...
        private IEnumerable<string> PossibleNames(string memberName, IEnumerable<string> prefixes, IEnumerable<string> postfixes)
        {
            ...
            foreach (var withoutPrefix in prefixes.Where(prefix => memberName.StartsWith(prefix, StringComparison.Ordinal)).Select(prefix => memberName.Substring(prefix.Length)))
            {
                ...
                foreach (var s in this.PostFixes(postfixes, withoutPrefix))
                    yield return s;
            }
            foreach (var s in this.PostFixes(postfixes, memberName))
                yield return s;
        }
    ...
    public class ReplaceName : ISourceToDestinationNameMapper
    {
        ...
        public MemberInfo GetMatchingMemberInfo(IGetTypeInfoMembers getTypeInfoMembers, TypeDetails typeInfo, Type destType, string nameToSearch)
        {
            var possibleSourceNames = this.PossibleNames(nameToSearch);
            var possibleDestNames = getTypeInfoMembers.GetMemberInfos(typeInfo).Select(mi => new { mi, possibles = this.PossibleNames(mi.Name) });
        ...
        {
            return 
                this.MemberNameReplacers.Select(r => nameToSearch.Replace(r.OriginalValue, r.NewValue))
                    .Concat(new[] { this.MemberNameReplacers.Aggregate(nameToSearch, (s, r) => s.Replace(r.OriginalValue, r.NewValue)), nameToSearch })
                    .ToList();
        }
    }
    ...
    public class MemberConfiguration : IMemberConfiguration
    {
        ...
        public IMemberConfiguration AddMember<TMemberMapper>(Action<TMemberMapper> setupAction = null)
            where TMemberMapper : IChildMemberConfiguration, new()
        {
            this.GetOrAdd(_ => (IList)_.MemberMappers, setupAction);
            return this;
        }
        ...
        public IMemberConfiguration AddName<TNameMapper>(Action<TNameMapper> setupAction = null)
            where TNameMapper : ISourceToDestinationNameMapper, new()
        {
            this.GetOrAdd(_ => (IList)_.NameMapper.NamedMappers, setupAction);
            return this;
        }
        ...
        public MemberConfiguration()
        {
            ...
            this.MemberMappers.Add(new DefaultMember { NameMapper = this.NameMapper });
        }
    ...
    public class NameSplitMember : IChildMemberConfiguration
    {
        ...
        {
            string[] matches = this.DestinationMemberNamingConvention.SplittingExpression
                       .Matches(nameToSearch)
                       .Cast<Match>()
                       .Select(m => this.SourceMemberNamingConvention.ReplaceValue(m))
                       .ToArray();
            MemberInfo matchingMemberInfo = null;
            for (int i = 1; i <= matches.Length; i++)
            {
                NameSnippet snippet = this.CreateNameSnippet(matches, i);
        ...
        {
            var first = string.Join(this.SourceMemberNamingConvention.SeparatorCharacter, matches.Take(i).Select(s => this.SourceMemberNamingConvention.SplittingExpression.Replace(s, this.SourceMemberNamingConvention.ReplaceValue)).ToArray());
            var second = string.Join(this.SourceMemberNamingConvention.SeparatorCharacter, matches.Skip(i).Select(s => this.SourceMemberNamingConvention.SplittingExpression.Replace(s, this.SourceMemberNamingConvention.ReplaceValue)).ToArray());
    ...
    public class DefaultMember : IChildMemberConfiguration
    {
        ...
        public bool MapDestinationPropertyToSource(IProfileConfiguration options, TypeDetails sourceType, Type destType, string nameToSearch, LinkedList<IValueResolver> resolvers, IMemberConfiguration parent = null)
        {
            if (string.IsNullOrEmpty(nameToSearch))
                return true;
            var matchingMemberInfo = this.NameMapper.GetMatchingMemberInfo(sourceType, destType, nameToSearch);
