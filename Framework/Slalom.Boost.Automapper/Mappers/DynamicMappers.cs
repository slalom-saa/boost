...
namespace Slalom.Boost.AutoMapper.Mappers
{
    public abstract class DynamicMapper : IObjectMapper
    {
        ...
        public object Map(ResolutionContext context)
        {
            ...
            foreach(var member in this.MembersToMap(source, destination))
            {
                object sourceMemberValue;
                try
                {
                    sourceMemberValue = this.GetSourceMember(member, source);
                }
                ...
                this.SetDestinationMember(member, destination, destinationMemberValue);
            }
            return destination;
        }
    ...
    public class FromDynamicMapper : DynamicMapper
    {
        ...
        {
            return this.GetDynamically(member, target);
        }
    ...
    public class ToDynamicMapper : DynamicMapper
    {
        ...
        protected override void SetDestinationMember(MemberInfo member, object target, object value)
        {
            this.SetDynamically(member, target, value);
        }
    }
}
